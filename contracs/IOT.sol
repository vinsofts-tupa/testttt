...
contract Warranty is People {
    ...
    function initWarranty(
        ...
        convention = Convention(_expireTime, _idProduct, _compensation, msg.sender, 0x0, 0x0, "", "");
    }
    ...
    function demonstrateMeInvalid(address _owner) public view returns(bool) {
        ...
        if(_owner == convention.buyer) {
            bool demonstrateEmployerInvalid = 
                EC.ecrecovery(convention.hash, convention.signatureEmployer) == convention.employer;
            require(demonstrateEmployerInvalid);
            
            return EC.ecrecovery(convention.hash, convention.signatureBuyer) == _owner;
        }
        if(_owner == convention.employer) {
            bool demonstrateBuyerInvalid = 
                EC.ecrecovery(convention.hash, convention.signatureBuyer) == convention.buyer;
            require(demonstrateBuyerInvalid);
            return EC.ecrecovery(convention.hash, convention.signatureEmployer) == _owner;
        }
